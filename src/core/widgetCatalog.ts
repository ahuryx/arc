import type { WidgetId } from "./types";

/** Widget metadata only — no React (controller-safe). */
export interface WidgetMeta {
  id: WidgetId;
  label: string;
  title: string;
  /** Content area height; window chrome added in widgetManager. */
  height: number;
}

/**
 * Fitted content heights (no scroll).
 * Calendar ≈ pad16+nav28+gaps16+weekdays20+6×28+10 ≈ 258 → 260
 * Clock ≈ time52 + dates + padding → 188
 */
export const WIDGET_CATALOG: WidgetMeta[] = [
  { id: "clock", label: "widget-clock", title: "Clock", height: 188 },
  { id: "calendar", label: "widget-calendar", title: "Calendar", height: 260 },
  { id: "crypto", label: "widget-crypto", title: "Crypto", height: 392 },
  { id: "weather", label: "widget-weather", title: "Weather", height: 176 },
  { id: "notes", label: "widget-notes", title: "Notes", height: 268 },
  { id: "pomodoro", label: "widget-pomodoro", title: "Pomodoro", height: 236 },
];

export const WIDGET_META_BY_ID = Object.fromEntries(
  WIDGET_CATALOG.map((widget) => [widget.id, widget]),
) as Record<WidgetId, WidgetMeta>;

export function getWidgetLabel(id: WidgetId): string {
  return WIDGET_META_BY_ID[id].label;
}
